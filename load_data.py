...
def load_surface_data(seed, subject_wise, split=0.3):
    ...
    for i in output_parameters:
        ...
        Ndata[f'{i}_Val'] = np.sqrt(
            np.array(Ndata[idx[0]]) ** 2 + np.array(Ndata[idx[1]]) ** 2 + np.array(Ndata[idx[2]]) ** 2)
    ...
    for i in Ndata.keys():
        ...
        X_tr, Y_tr, X_te, Y_te, P_tr, P_te = subject_wise_split(np.array(Ndata[i]), np.array(Surface),
                                                                np.array(Participant), subject_wise=subject_wise,
    ...
    KEY = 'Lower'
    return AnnData['X_train'][KEY], Lab.one_hot(AnnData['y_train'][KEY]), P_tr, AnnData['X_test'][KEY], Lab.one_hot(
        AnnData['y_test'][KEY]), P_te
